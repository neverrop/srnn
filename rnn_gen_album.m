function [seqs,lls,seqsind,llsall] = rnn_gen_album(net,album,dpp,varargin)
...
% Carnegie Mellon University
    ...
    if dpp==1
        dp = py.list(alb(:)');
        re = py.dpp.sample_hierarchy_stream(dp,4097,opts.length);
    end
    for r=1:opts.num %multiple runs
        ...
        % set up network
            ...
            if dpp == 1
            %initial network with DPP
                for t=2:(opts.length-1)
                    input{1}(:) = alb(re{t-1},:);
                    [hidden] = rnn_forward(input, hidden, net, opts.actCut);
                end
            end
            for j=2:opts.length 
