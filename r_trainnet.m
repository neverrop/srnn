...
for t = 1:length(gtea)
    ...
    for dpp = 1:2
        ...
        for c = 1:2
            ...
            fePath1 = strcat(path1,'*_features.mat');
            %a = ['*_',cc,'features.mat'];
            fePath = dir(fePath1);
            fePath = fePath(c:2:end);
            fe = {};
            for jj = 1:length(fePath)-1
                load(strcat(path1,fePath(jj).name));
                se = ceil(size(features,1)/5000);
                A1 = album(features,24,zeros(length(features)),12*se);
                A2 = album(features,24,zeros(length(features)),24*se);
                for jjj = 1:length(A1)
                    fe = [fe,A1{jjj},A2{jjj}];
                end
                %fe = [fe,A1,A2];
            end
            %save(strcat(path1, ['all_' fename 'fe.mat']),'fe');

            A = fe;
            l = length(A);
            subsampl = 15+2; %Size of the story. Low number: Easy to learn, but not very informative. High number: hard to learn.
            vocab_size = 4096;
            input_size = 4096+1;
            hidden_size = 50; 
            ...
            % Initialize with DPP
            if dpp == 1
                fprintf('Initialize with DPP.')
                alb = [EOS; [A2{1} zeros(size(A2{1},1),1)]; EOS]/maxval; 
                dp = py.list(alb(:)');
                re = py.dpp.sample_hierarchy_stream(dp,4097,subsampl-2);
                ...
                for i=repmat(validid,1,repeatVal)
                    ...
                    for j=1:subsampl
                        ...
                        elseif j == subsampl
                            ...
                            selw = gather(ind(sum(rand(1) > cumprob) + 1));
                        end
                        seq(j+1) = selw+seq(j);
            ...
            %     name = strcat(imgpath,sprintf('summaryTurn_%d',iter));
            %     showSummaries(seq(3:end-1),imgpath,name);
            ...
            if dpp == 1
                save(strcat(path1,gtea(t).name,'_',cc,num2str(subsampl-2),'_dpp_model.mat'),'net');
            else
                save(strcat(path1,gtea(t).name,'_',cc,num2str(subsampl-2),'_model.mat'),'net');
            end
        end
    end
end