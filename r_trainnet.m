...
for t = 3:length(gtea)
    ...
    for c = 1:2
        ...
        for j = 1:length(fePath)
            ...
            A1 = album(features,12,zeros(length(features)));
            fe = [fe , A1];
        end
        ...
        A = fe;
        l = length(A);
        subsampl = 20+2; %Size of the story. Low number: Easy to learn, but not very informative. High number: hard to learn.
        vocab_size = 4096;
        input_size = 4096+1;
        hidden_size = 50; 
        ...
        save(strcat(path1,gtea(t).name,'_',cc,'model.mat'),'net');
    end
end